...
def load_test_data(test_path):
    test_img_list = glob(os.path.join(test_path, '**', '*.png'), recursive=True)
    ...
    for img in test_img_list:
        ...
        new_data = pd.DataFrame({"path": img, "label": label_name, "class_id": label_list.index(label_name)}, index=[0])
...
def evaluate_test_set(dataloader, model):
    ...
    classification_rep = classification_report(all_labels, all_preds, target_names=[label_list[i] for i in unique_labels])
...
def main():
    ...
    sns.heatmap(test_confusion_matrix, annot=True, fmt='d', cmap='Blues', xticklabels=[label_list[i] for i in range(len(test_confusion_matrix))], yticklabels=[label_list[i] for i in range(len(test_confusion_matrix))])
